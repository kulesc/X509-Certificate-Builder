...
public class CertificateSignWindow extends JDialog {
	...
	private X509CertificateBuilder x509Builder;
	private KeyPair pair;
	public CertificateSignWindow(KeyPairInfo k,
			org.bouncycastle.pkcs.PKCS10CertificationRequest c, X509CertificateBuilder x509b, KeyPair p) {
		pair = p;
		x509Builder = x509b;
		csr = c;
		setModal(true);
		...
		btnClose.setBounds(401, 617, 98, 24);
		...
		if (kpi.getBci() == null)
			lblBasicConstraints
					.setText("Basic Constraints: No Basic Constraints");
		else
			lblBasicConstraints.setText(kpi.getBci().toString());
		...
		btnSign.setBounds(302, 618, 89, 23);
		getContentPane().add(btnSign);

		if (kpi.getKui() == null)
			txtrKeyusage.setText("No Key Usage Extension");
		else
			txtrKeyusage.setText(kpi.getKui().toString());
		...
		btnSign.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					new CertificateSignature(theCert).setVisible(true);
					kpi.setSigned(true);
					kpi.setSignature(theCert.getSignature());
					kpi.setSigAlg(theCert.getSigAlgName());
					x509Builder.putCert(kpi.getAlias(), theCert);
